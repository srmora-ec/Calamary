...
import { CheckCircle2, Calculator, HelpCircle, Settings } from "lucide-react"
...
interface FuzzyScaleItem {
  label: string
  value: FuzzyNumber
}

const DEFAULT_FUZZY_SCALE: Record<number, FuzzyScaleItem> = {
  0: { label: "Igual", value: { l: 1, m: 1, u: 1 } },
  1: { label: "Intermedio", value: { l: 1, m: 2, u: 3 } },
  2: { label: "Moderada", value: { l: 2, m: 3, u: 4 } },
  3: { label: "Intermedio", value: { l: 3, m: 4, u: 5 } },
  4: { label: "Fuerte", value: { l: 4, m: 5, u: 6 } },
  5: { label: "Intermedio", value: { l: 5, m: 6, u: 7 } },
  6: { label: "Muy Fuerte", value: { l: 6, m: 7, u: 8 } },
  7: { label: "Intermedio", value: { l: 7, m: 8, u: 9 } },
  8: { label: "Absoluta", value: { l: 9, m: 9, u: 9 } },
}
...
const ComparacionParesDifusos: React.FC<ComparacionParesDifusosProps> = ({ nodos, onSave, onCancel }) => {
  ...
  const [fuzzyScale, setFuzzyScale] = useState<Record<number, FuzzyScaleItem>>(DEFAULT_FUZZY_SCALE)
  const [showScaleModal, setShowScaleModal] = useState(false)
  const [editableScale, setEditableScale] = useState<Record<number, FuzzyScaleItem>>(DEFAULT_FUZZY_SCALE)
    ...
    nodos.forEach((rowNode) => {
      nodos.forEach((colNode) => {
        if (rowNode.idnodo === colNode.idnodo) {
          ...
          const scaleVal = fuzzyScale[absVal]?.value || { l: 1, m: 1, u: 1 }
  ...
  const handleOpenScaleModal = () => {
    setEditableScale(JSON.parse(JSON.stringify(fuzzyScale)))
    setShowScaleModal(true)
  }

  const handleUpdateScaleValue = (index: number, field: "l" | "m" | "u", value: string) => {
  ...
  const handleUpdateScaleLabel = (index: number, label: string) => {
    setEditableScale((prev) => ({
      ...prev,
      [index]: {
        ...prev[index],
  ...
  const handleSaveScale = () => {
    setFuzzyScale(editableScale)
    setShowScaleModal(false)
  }

  const handleResetScale = () => {
    setEditableScale(DEFAULT_FUZZY_SCALE)
  }

  const getSliderLabel = (val: number) => {
    if (val === 0) return "1"
    const abs = Math.abs(val)
    const fuzzyStr = `(${fuzzyScale[abs].value.l},${fuzzyScale[abs].value.m},${fuzzyScale[abs].value.u})`
    return fuzzyStr
  }
  ...
  return (
    <div className="w-full space-y-6 animate-in fade-in duration-500">
      <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-lg p-4 text-sm text-muted-foreground">
        ...
        <p>
          Deslice hacia el lado del criterio más importante. El cálculo aplicará el Método de Buckley: Media Geométrica
          por Fila (Izquierda - Derecha).
        </p>
      </div>

      <div className="flex justify-end">
        <Button variant="outline" onClick={handleOpenScaleModal} className="gap-2 bg-transparent">
            ...
            <div key={idx} className="border rounded-lg p-4 sm:p-6 hover:bg-accent/5 transition-colors">
              <div className="flex items-center justify-between gap-4 mb-6">
                <div className="flex-1 bg-blue-50 dark:bg-blue-950/40 p-2 rounded border border-blue-100 dark:border-blue-900 flex items-center gap-2">
                  ...
                  {val < 0 && (
                    <span className="text-xs font-mono text-blue-600 bg-blue-100 px-1 rounded hidden sm:inline-block">
                      Val: {getSliderLabel(val)}
                    </span>
                  )}
                </div>
                ...
                <div className="flex-1 bg-green-50 dark:bg-green-950/40 p-2 rounded border border-green-100 dark:border-green-900 flex items-center gap-2 text-right justify-end">
                  {val > 0 && (
                    <span className="text-xs font-mono text-green-600 bg-green-100 px-1 rounded hidden sm:inline-block">
                      Val: {getSliderLabel(val)}
                    </span>
                  )}
      ...
      <div className="flex justify-end gap-4 pt-4 border-t">
        ...
        <Button onClick={handleCalculate} disabled={loading} size="lg" className="min-w-[150px]">
          {loading ? (
            ...
            <>
              <Calculator className="mr-2 h-4 w-4" /> Calcular Pesos Difusos
            </>
          )}
        </Button>
      </div>
      ...
      <Modal
        ...
        title="Configuración de Escala Difusa"
        width="700px"
      >
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Personaliza los valores triangulares difusos (l, m, u) para cada nivel de la escala de Saaty.
          </p>

          <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
            {Object.keys(editableScale).map((key) => {
              const index = Number.parseInt(key)
              const item = editableScale[index]
                  ...
                  <CardContent className="pt-4">
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-sm font-bold text-muted-foreground min-w-[30px]">{index}:</span>
                        <div className="flex-1">
                          <Label className="text-xs text-muted-foreground">Etiqueta</Label>
                          <Input
                            value={item.label}
                            onChange={(e) => handleUpdateScaleLabel(index, e.target.value)}
                            className="h-8"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 pl-9">
                        <div>
                          <Label className="text-xs text-muted-foreground">L (bajo)</Label>
                          <Input
                            type="number"
                            step="0.1"
                            min="0"
                            value={item.value.l}
                            onChange={(e) => handleUpdateScaleValue(index, "l", e.target.value)}
                            className="h-8 font-mono"
                          />
                        </div>
                        <div>
                          <Label className="text-xs text-muted-foreground">M (medio)</Label>
                          <Input
                            type="number"
                            step="0.1"
                            min="0"
                            value={item.value.m}
                            onChange={(e) => handleUpdateScaleValue(index, "m", e.target.value)}
                            className="h-8 font-mono"
                          />
                        </div>
                        <div>
                          <Label className="text-xs text-muted-foreground">U (alto)</Label>
                          <Input
                            type="number"
                            step="0.1"
                            min="0"
                            value={item.value.u}
                            onChange={(e) => handleUpdateScaleValue(index, "u", e.target.value)}
                            className="h-8 font-mono"
                          />
                        </div>
                      </div>

                      <div className="text-xs text-center font-mono text-muted-foreground pl-9">
                        ({item.value.l}, {item.value.m}, {item.value.u})
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          <div className="flex justify-between gap-2 pt-4 border-t">
            <Button variant="outline" onClick={handleResetScale} size="sm">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleSaveScale} className="bg-green-600 hover:bg-green-700">
                ...
                <TableBody>
                  ...
                  <TableRow className="bg-blue-50/50 font-medium border-t-2 border-blue-100">
                    ...
                    <TableCell className="text-center font-mono text-sm">
                      {totalGeometricMean.l.toFixed(2)} &nbsp; {totalGeometricMean.m.toFixed(2)} &nbsp;{" "}
                      {totalGeometricMean.u.toFixed(2)}
                    </TableCell>
                  </TableRow>
                  ...
                  <TableRow className="bg-slate-50/50 text-muted-foreground text-xs">
                    ...
                    <TableCell className="text-center font-mono">
                      {rawInverseDisplay.l.toFixed(2)} &nbsp; {rawInverseDisplay.m.toFixed(2)} &nbsp;{" "}
                      {rawInverseDisplay.u.toFixed(2)}
                    </TableCell>
                  </TableRow>
                  ...
                  <TableRow className="bg-green-50/50 text-green-800 font-medium">
                    ...
                    <TableCell className="text-center font-mono text-sm">
                      {orderedInverse.l.toFixed(2)} &nbsp; {orderedInverse.m.toFixed(2)} &nbsp;{" "}
                      {orderedInverse.u.toFixed(2)}
                    </TableCell>
                  </TableRow>
                </TableBody>
              </Table>
